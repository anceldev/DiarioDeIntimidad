...
class ApiService {
    ...
    init() {
        for book in ListBooks.allCases {
            print(book.spanish)
        }
    }
    
    func makeRequest<T: Codable>(_ apiRoute: ApiRoutes, _ book: ListBooks? = nil) async throws -> T {
        let includesBook = book != nil ? book!.rawValue : ""
        let absoluteRoute = apiRoute.fullRoute + includesBook
        
        do {
            let data: T = try await fetchItem(route: absoluteRoute)
            return data
        } catch {
